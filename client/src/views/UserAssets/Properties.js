...
const Properties = ({props}) => {
  ...
  const removeModal = async (idToRemove) => {
    let response = await api('/api/assets/deleteasset', {id: idToRemove, assetName: "Property"});
    if (response.error) {
        ...
        <Stack direction='row' spacing='4'>
          ...
          {modals.map((modal) => (
            <Box key={modal.id} borderWidth='1px' borderRadius='md' p='4' boxShadow='md'>
              <OpenModal keyId={modal.id} inputData={modal.inputData} assetName="Property" url = {"/api/assets/updateaddassets"} ReqVal={"Value"} OptString={"OptString"} FormVals={["Enter Property Name", "Set your Asset Assignment", "Enter Property Value", "Enter Specific Clauses (optional)"] } />
